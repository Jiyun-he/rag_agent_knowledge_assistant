...
本 README 不写入具体检索策略的优劣结论。此前一版基于 12 个 EvalCase 的评测数据（Recall@K / Hit@K / MRR / Citation Correct Rate 对比表）已作废，原因是其中的 `Citation Correct Rate` 指标已不再存在，混合检索的融合算法也已改为等权 RRF，每路召回深度、重排候选数等都发生了变化，旧数字与当前实现不可比。
